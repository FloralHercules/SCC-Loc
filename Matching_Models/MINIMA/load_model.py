...
import os
import sys
# 获取当前文件的目录 (Matching_Models/MINIMA)
MINIMA_ROOT = os.path.dirname(os.path.abspath(__file__))
if MINIMA_ROOT not in sys.path:
    sys.path.append(MINIMA_ROOT)
def load_roma(args, test_orginal_megadepth=False):
    roma_path = os.path.join(MINIMA_ROOT, "third_party", "RoMa_minima")
    if roma_path not in sys.path:
        sys.path.append(roma_path)
...
def load_loftr(args, test_orginal_megadepth=False):
    loftr_path = os.path.join(MINIMA_ROOT, "third_party", "LoFTR_minima", "src")
    if loftr_path not in sys.path:
...
def load_sp_lg(args, test_orginal_megadepth=False):
    lg_path = os.path.join(MINIMA_ROOT, "third_party", "LightGlue")
    if lg_path not in sys.path:
...
def load_xoftr(args, test_orginal_megadepth=False):
    xoftr_path = os.path.join(MINIMA_ROOT, "third_party", "XoFTR", "src")
    if xoftr_path not in sys.path:
        sys.path.append(xoftr_path)
...
def load_model(method, args, use_path=True, test_orginal_megadepth=False, return_object=False):
    # if use_path:
    ...
    matcher = eval(f"load_{method}")(args, test_orginal_megadepth=test_orginal_megadepth) # TODO
