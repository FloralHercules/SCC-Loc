...
class homo_dataset(data.Dataset):
    ...
    def __getitem__(self, query_PIL_image, database_PIL_image, query_utm, database_utm, index, pos_index, neg_img2=None):
        ...
        if torch.is_tensor(img2):
            ref_size = float(min(img2.shape[-2], img2.shape[-1]))
        elif hasattr(img2, 'size'):
            if isinstance(img2.size, tuple):
                ref_size = float(min(img2.size[0], img2.size[1]))
            else:
                ref_size = float(img2.size)
        else:
            ref_size = float(self.args.resize_width)
        alpha = ref_size / float(self.args.resize_width)
        ...
        if self.augment:
            ...
            use_center_crop = bool(getattr(self.args, 'use_query_center_crop', False))
            beta = self.args.crop_width / self.args.resize_width if use_center_crop else 1.0
        ...
        if bool(getattr(self.args, 'use_query_center_crop', False)):
...
class MYDATA(homo_dataset):
    ...
    def _find_img_in_map(self, index, database_queries_split=None):
        ...
        sat_size_px = int(getattr(self.args, 'sat_size_px', getattr(self.args, 'database_size', 512)))
        center_cood = (float(image_name.split("@")[1]), float(image_name.split("@")[2]))
        area = (int(center_cood[1]) - sat_size_px//2, int(center_cood[0]) - sat_size_px//2,
                int(center_cood[1]) + sat_size_px//2, int(center_cood[0]) + sat_size_px//2)
