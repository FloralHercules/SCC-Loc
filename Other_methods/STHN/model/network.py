...
class STHN():
    def __init__(self, args, for_training=False):
        ...
        self.four_point_org_large_single[:, :, 0, 1] = torch.Tensor([self.sat_size_px - 1, 0]).to(self.device)
        self.four_point_org_large_single[:, :, 1, 0] = torch.Tensor([0, self.sat_size_px - 1]).to(self.device)
        self.four_point_org_large_single[:, :, 1, 1] = torch.Tensor([self.sat_size_px - 1, self.sat_size_px - 1]).to(self.device) # Only to calculate flow so no -1
    ...
    def get_cropped_st_images(self, image_1_ori, four_pred, fine_padding, detach=True, augment_two_stages=0):
        ...
        alpha = self.sat_size_px / self.args.resize_width
        x[:, :, 0] = x[:, :, 0] * alpha
        x[:, :, 1] = (x[:, :, 1] + 1) * alpha
        y[:, 0, :] = y[:, 0, :] * alpha
        y[:, 1, :] = (y[:, 1, :] + 1) * alpha
